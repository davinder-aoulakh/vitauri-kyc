              ...
              <div className="bg-card border border-border rounded-xl overflow-hidden">
                <div className="px-4 py-2.5 border-b border-border">
                  <span className="text-xs text-muted-foreground font-medium">
                    Recent clients — {allClients.length} total · type to filter
                  </span>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                            ...
                            <tr key={client.id} className="hover:bg-muted/30 transition-colors">
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                                  ...
                                  {isRestricted && (
                                    <span className="flex items-center gap-1 text-xs text-red-600 font-medium bg-red-50 px-1.5 py-0.5 rounded-full border border-red-200 whitespace-nowrap">
                                      <AlertTriangle className="w-3 h-3" /> Gatekept
                                    </span>
                                  )}
                                </div>
                                {client.registration_number && (
                ...
                {allClients.length > 20 && (
                  <div className="px-4 py-2.5 border-t border-border text-xs text-muted-foreground text-center">
                    Showing 20 most recent — type to search all {allClients.length} clients
                  </div>
                )}
              </div>
            ) : (
